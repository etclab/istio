...
// IsBenchmarkInlineEnabled checks whether inline benchmark mode is enabled.
// Reads /etc/mazu-config/MAZU_BENCHMARK_INLINE_ENABLED. Disabled by default.
func IsBenchmarkInlineEnabled() bool {
	filepath := fmt.Sprintf("%s/%s", MAZU_CONFIG_PATH, MAZU_BENCHMARK_INLINE_ENABLED)
	content, err := os.ReadFile(filepath)
	if err != nil {
		log.Infof("[dev] benchmark inline mode is disabled, file %s not found", filepath)
		return false
	}
	if strings.TrimSpace(string(content)) != "true" {
		log.Infof("[dev] benchmark inline mode is disabled, with value: %s", string(content))
		return false
	}
