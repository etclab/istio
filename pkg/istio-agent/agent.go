...
func (a *Agent) initializeEnvoyAgent(_ context.Context) error {
	...
	podName := node.Metadata.InstanceName
	log.Infof("[dev] pod name: %s", podName)
...
func (a *Agent) getRbeUserId() (*kcUtil.RbeId, error) {
	...
	// TODO: instead of using a random nonce, use the pod uid generated by k8s
	uid, err := kcUtil.GetPodUid()
	if err != nil {
		log.Errorf("[dev] failed to get pod uid: %v", err)
	}
	log.Infof("[dev] pod uid: %s", uid)
